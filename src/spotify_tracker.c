...
int format_file(artist *artists, int artists_len, song *songs, int songs_len, char **file_content_out) {
    ...
    if (append_string(main_header, content, current_size, pos, &pos, &current_size, &content) != STATUS_SUCCESS) {
        free(content);
        return STATUS_ERROR;
    }
    ...
    if (append_string(app_info, content, current_size, pos, &pos, &current_size, &content) != STATUS_SUCCESS) {
        free(content);
        return STATUS_ERROR;
    }
